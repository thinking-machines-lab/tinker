...
from .retry_handler import RetryableException, RetryConfig, RetryHandler
...
def not_none(x: T | None) -> T:  # noqa: UP047
...
class ResolvedFuture(Generic[T], ConcurrentFuture[T]):  # noqa: UP046
...
class AwaitableConcurrentFuture(Generic[T]):  # noqa: UP046
...
class APIFuture(ABC, Generic[T]):  # noqa: UP046
    @abstractmethod
        ...
        async def _create_model_async():
            ...
            with self.holder.aclient() as client:
                future = await client.models.create(
                    base_model=base_model, lora_config=to_lora_config_params(lora_config)
                )
